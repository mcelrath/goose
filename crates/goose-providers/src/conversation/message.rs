...
impl Message {
    ...
    /// Add text content that is visible to the agent but not the user.
    /// The MCP `audience` field restricts this content to `[Role::Assistant]` so
    /// `filter_for_audience` strips it when rendering the user side.
